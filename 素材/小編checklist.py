...
點擊會即時記錄到 素材/小編進度.json（搭檔／程式都讀得到）。

啟動：
    streamlit run 素材/小編checklist.py
要讓遠端的小編也能開，見 素材/小編checklist_使用說明.md。
"""
import os
import json
import datetime
...
PROGRESS_FILE = os.path.join(HERE, '小編進度.json')
YEAR = 2026
...
# 把 session_state 從存檔初始化（避免覆蓋使用者剛點的）
...
# 點擊回呼已把最新狀態寫進 json，這裡重讀一次，確保進度數字與「已完成時間」即時更新
prog = load_progress()
...
# 全部（依階段，今天的自動展開）；已在上方顯示過的就不再放 checkbox
st.markdown('---')
st.header('全部任務')
for ph in phases:
    is_today = d(ph['起']) <= today <= d(ph['迄'])
